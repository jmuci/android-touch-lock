...
| B2 | ~~No hosted Privacy Policy URL~~ | — | **Done** — GitHub Pages enabled 2026-08-30; confirmed live at `https://jmuci.github.io/android-touch-lock/privacy-policy.html` |
...
| B7 | **New finding, 2026-08-30**: no in-app link to the Privacy Policy — only planned in Play Console's App content field | Confirmed against current Play policy: apps requesting a sensitive permission (Accessibility qualifies) must link to the privacy policy both on the store listing *and* from within the app itself, typically via a menu. Play Console's field alone is not sufficient. Checked the codebase: there is currently only one screen (`HomeScreen.kt`) and no About/Settings surface at all | Add a small text link (e.g. "Privacy Policy") to the bottom of `HomeScreen`, opening `https://jmuci.github.io/android-touch-lock/privacy-policy.html` via `Intent.ACTION_VIEW` — matches the app's existing minimal-surface convention rather than introducing a new About screen for one link. Not yet implemented — pending confirmation |
...
- [x] **C3** — ~~Generate an upload keystore; wire `signingConfigs.release`~~ — done (2026-08-30): keystore generated, `keystore.properties` filled in, signed AAB verified (B1).
